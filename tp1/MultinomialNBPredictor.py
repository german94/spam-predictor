...
#Directorio resultados con subdirectorio naiveBayesMultinom, dentro de naiveBayesMultinom tiene que estar el archivo best_mnb_clf
...
y = df['class']
mnb_clf = pickle.load(open("resultados/naiveBayesMultinom/best_mnb_clf", "rb"))

startTimeAttSelection = time.time()
best = SelectKBest(f_classif, k=200)
...
file = open("resultados/naiveBayesMultinom/best", "wb")
pickle.dump(best, file)
file.close()
print "Ejecutando Naive Bayes Multinomial con los mejores 200 atributos..."
best_attrs_res = cross_val_score(mnb_clf, X_new, y, cv=10)
...
file = open("resultados/naiveBayesMultinom/clasificadorEntrenado_200att", "wb")
...
predArr = mnb_clf.predict(X_new_test)
...
print "Accuracy de Naive Bayes Multinomial sobre test: " + str(accuracy_score(predArr, y_test))
print "f1_score de Naive Bayes Multinomial sobre test: " + str(f1_score(predArr, y_test, pos_label="spam"))