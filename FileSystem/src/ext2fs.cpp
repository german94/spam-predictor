...
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int addresses_per_block = block_size/sizeof(unsigned int);
	...
	else if (block_number < addresses_per_block+12) //indireccion simple
	{
		unsigned int single_indirect_table_address = inode->block[12];
		read_block(single_indirect_table_address, buffer);
		res = ((unsigned int *)buffer)[block_number-12];
	}
	else if (block_number < (addresses_per_block*addresses_per_block)+12) //indireccion doble
	{
